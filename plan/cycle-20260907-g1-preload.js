(function(){
  const TARGET_PLAN_FILES = new Set([
    "plan_sr_jeremias.json",
    "plan_mario_lopez.json",
    "plan_evan_mael.json",
    "plan_daniele_biasetti.json",
    "plan_mtro_mariano_agustina.json",
    "plan_rodrigo_jalisco.json"
  ]);
  ...
  function targetFileFromUrl(value){
    const url = String(value || "");
    const match = url.match(/data\/planes\/([^?#/]+\.json)/);
  ...
  window.fetch = async function(input, init){
    ...
    try{
      ...
      const planFile = targetFileFromUrl(url);
      if(TARGET_PLAN_FILES.has(planFile)){
