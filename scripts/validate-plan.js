 ...
 *
 * Este script no modifica archivos. Solo reporta errores.
 */
...
function readJson(relativePath) {
  ...
  catch (error) { fail(`No se pudo leer JSON en ${relativePath}: ${error.message}`); }
}
function isNonEmptyString(value) { return typeof value === 'string' && value.trim().length > 0; }
function resourceKey(title, url, mediaType) { return `${title}\u0000${url}\u0000${mediaType}`; }
function pushError(errors, location, message) { errors.push(`${location}: ${message}`); }
function walkStrings(value, location, visit) {
  if (typeof value === 'string') { visit(value, location); return; }
  if (Array.isArray(value)) { value.forEach((item, index) => walkStrings(item, `${location}[${index}]`, visit)); return; }
  if (value && typeof value === 'object') Object.entries(value).forEach(([key, nested]) => walkStrings(nested, `${location}.${key}`, visit));
}
...
function buildMediaPolicy() {
  ...
  (review.approved_additions_files || []).forEach((relativePath) => {
    const addition = readJson(relativePath);
    (addition.resources || []).forEach((resource) => {
      addAllowed(resource.title, resource.url, resource.media_type, Boolean(resource.is_replacement));
    });
  });
...
function validateTopLevel(plan, errors) {
  if (!plan || typeof plan !== 'object' || Array.isArray(plan)) { pushError(errors, '$', 'el plan debe ser un objeto JSON.'); return; }
  REQUIRED_PLAN_KEYS.forEach((key) => { if (!Object.prototype.hasOwnProperty.call(plan, key)) pushError(errors, '$', `falta la llave obligatoria "${key}".`); });
  Object.keys(plan).forEach((key) => { if (!REQUIRED_PLAN_KEYS.includes(key) && !OPTIONAL_PLAN_KEYS.includes(key)) pushError(errors, `$.${key}`, 'llave no reconocida por el contrato del plan.'); });
  ['updated_at', 'nivel', 'ciclo', 'enfoque_corto', 'enfoque', 'chanonaflexDias', 'isometricoDias', 'pateoDias', 'poomsaeDias'].forEach((key) => {
    if (Object.prototype.hasOwnProperty.call(plan, key) && !isNonEmptyString(plan[key])) pushError(errors, `$.${key}`, 'debe ser un texto no vacío.');
  });
  [...RESOURCE_SECTIONS, ...TEXT_ARRAY_KEYS].forEach((key) => {
    if (Object.prototype.hasOwnProperty.call(plan, key) && !Array.isArray(plan[key])) pushError(errors, `$.${key}`, 'debe ser un array.');
  });
  TEXT_ARRAY_KEYS.forEach((key) => {
    if (!Array.isArray(plan[key])) return;
    plan[key].forEach((item, index) => { if (!isNonEmptyString(item)) pushError(errors, `$.${key}[${index}]`, 'debe ser un texto no vacío; no usar strings vacíos.'); });
  });
}

function validateResource(resource, section, index, mediaPolicy, errors) {
  const location = `$.${section}[${index}]`;
  if (!resource || typeof resource !== 'object' || Array.isArray(resource) || Object.keys(resource).length === 0) { pushError(errors, location, 'debe ser un objeto de recurso completo; no usar objetos vacíos.'); return; }
  RESOURCE_REQUIRED_KEYS.forEach((key) => {
    if (!Object.prototype.hasOwnProperty.call(resource, key)) pushError(errors, location, `falta la llave obligatoria "${key}".`);
    else if (!isNonEmptyString(resource[key])) pushError(errors, `${location}.${key}`, 'debe ser un texto no vacío.');
  });
  Object.keys(resource).forEach((key) => { if (!RESOURCE_REQUIRED_KEYS.includes(key) && !RESOURCE_OPTIONAL_KEYS.includes(key)) pushError(errors, `${location}.${key}`, 'llave de recurso no reconocida.'); });
  ['dia', 'enfoque', 'reps'].forEach((key) => { if (Object.prototype.hasOwnProperty.call(resource, key) && !isNonEmptyString(resource[key])) pushError(errors, `${location}.${key}`, 'debe ser un texto no vacío cuando se incluye.'); });
  ...
  if (!registeredForUrl) { pushError(errors, `${location}.url`, 'URL no autorizada en la biblioteca oficial/revisión aprobada.'); return; }
  ...
  if ((resource.tipo === 'audio' || isAudio) && section !== 'poomsae') pushError(errors, location, 'los audios de timing solo pueden asignarse dentro de la sección poomsae.');
}
function validatePrivacy(plan, errors) {
  walkStrings(plan, '$', (text, location) => SENSITIVE_PATTERNS.forEach(({ label, regex }) => { if (regex.test(text)) pushError(errors, location, `posible ${label} en un repositorio público; revisar o retirar antes de publicar.`); }));
}
function main() {
  const planArg = process.argv[2];
  if (!planArg) { console.error('Uso: node scripts/validate-plan.js data/planes/plan_alumno.json'); process.exit(2); }
  const relativePlanPath = path.normalize(planArg).replace(/^\.\//, '');
  if (!relativePlanPath.startsWith(`data${path.sep}planes${path.sep}`) || !relativePlanPath.endsWith('.json')) fail('Solo se validan archivos JSON dentro de data/planes/.');
  const plan = readJson(relativePlanPath);
  const mediaPolicy = buildMediaPolicy();
  const errors = [];
  validateTopLevel(plan, errors);
  RESOURCE_SECTIONS.forEach((section) => { if (Array.isArray(plan[section])) plan[section].forEach((resource, index) => validateResource(resource, section, index, mediaPolicy, errors)); });
  ...
  if (errors.length > 0) {
    console.error('VALIDACIÓN FALLIDA:'); errors.forEach((error) => console.error(`- ${error}`)); console.error(`\nResultado: ${errors.length} error(es).`); process.exit(1);
  }
